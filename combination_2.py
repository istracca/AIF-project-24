...
def combination_2(MapGame, Joystick, Goal, SuccessorFunction):
    moves = 0
    ...
    while(1):
        ...
        if CharacterPosition==MonsterPosition:
            ...
            if res!=False:      # Esiste un percorso a rischio zero. Seguo quello fino alla fine
                ...
                #   1. Uso A* senza considerare il lupo e controllo se il primo step mi porterebbe in uno stato "non sicuro"
                path = a_star_search(CharacterPosition,Goal,SuccessorFunction)[0]
                print("Senza considerare i mostri, farei questo percorso:", path)
                next_monst_position = next_step_monster(MonsterPosition, path[0],SuccessorFunction)
                if a_star_prev_monst(path[0],Goal,next_monst_position,SuccessorFunction)[2] >= 10000:
                    print("La prima mossa che farei mi porterebbe in uno stato non sicuro")
                else:
                    print("Eseguo la prima mossa, dato che mi garantisce comunque un percorso sicuro")
                ...
                #   2. Ipotizzando che i lupi si muovano come descritto in next_step_monster, cerco un percorso sicuro almeno sotto tale ipotesi
